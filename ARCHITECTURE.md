...
| `Device` | Aparelho liberado para marcar presença sem QR rotativo. Código alfanumérico maiúsculo de 6 caracteres (`Device.generate_code()`, `CODE_ALPHABET`/`CODE_LENGTH`) por evento, resgatado uma única vez (`activated_at`), válido por `VALIDITY_DAYS` (30) e cortável (`revoked_at`). É a credencial que vai no path das rotas `/api/checkin/device/...`. |
...
- `api/` — wrappers de fetch por domínio (`authApi`, `checkinApi`, `deviceApi`, `memberApi`, `scoreApi`) sobre `callFetch` em `index.svelte.ts`. Cada wrapper devolve `ApiResponse<T> {success, message, data?: T, status?}`, tipado com o corpo esperado (`ScoreEntry[]`, `PendingMember[]`, `DevicePending` etc.); `status` carrega o HTTP real nas falhas do modo dispositivo. `callFetch` redireciona para `/login` em 401 quando `ensureLogin` (padrão `true`); rotas públicas passam `ensureLogin: false`. CSRF: `getCsrfToken()` lê o cookie `csrftoken` e só o upload de foto o envia (no corpo multipart).
...
- **Testes**: `docker-compose-test.yml` roda `pytest -q ../tests` dentro do container do back, com banco em `/tmp`. Localmente: `cd back && uv run python -m pytest ../tests`. Config em `pytest.ini` (raiz) + `conftest.py` (adiciona `back/` ao `sys.path`). Fixtures em `tests/conftest.py` (`user`, `member`, `event` com regra única de 50 pts, `code`, `expired_code`, `device`, `active_device`). Cobertura atual: API de check-in (inclusive `history` e `already`), auth (`login`/`logout`/`logged`), `per-event`, consumer WebSocket (`joinEvent`, erros), device (controller, API, admin com QR e ação `revoke`, concorrência da ativação), código/rotação/timer/retenção, concorrência do check-in, `Config`, `TimeScoreRules.clean`, `member/me` e upload de foto, pontuação, `didnt_checkin_today` (com o caso A1 do `REVIEW.md` marcado `xfail(strict=True)`) e os commands `import_checkins`/`export_checkins`. Não há testes do front.
